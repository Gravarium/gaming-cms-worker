...
export function credentialJson(credential) {
    if (!credential) {
        throw new Error('Der Passkey-Vorgang wurde nicht abgeschlossen.');
    }
...
export async function postJson(url, body, fetcher = globalThis.fetch) {
    ...
    if (!response.ok) {
        throw new Error('Die Passkey-Anfrage konnte nicht bestätigt werden.');
    }
...
export async function performPasskeyLogin({
    ...
    navigate = url => globalThis.window.location.assign(url),
    publicKeyCredential = globalThis.PublicKeyCredential,
}) {
    const normalizedUsername = username?.trim();
    if (!normalizedUsername) {
        throw new Error('Bitte zuerst die E-Mail-Adresse eingeben.');
    }
    if (!credentials?.get) {
        throw new Error('Dieser Browser unterstützt keine Passkeys.');
    }

    const options = await postJson(optionsUrl, {username: normalizedUsername}, fetcher);
    const credential = await credentials.get({publicKey: requestOptions(options, publicKeyCredential)});
    await postJson(resultUrl, credentialJson(credential), fetcher);
    navigate(successUrl);
}
...
export async function performPasskeyRegistration({
    ...
    const options = await postJson(optionsUrl, {}, fetcher);
    const credential = await credentials.create({publicKey: creationOptions(options, publicKeyCredential)});
    await postJson(resultUrl, credentialJson(credential), fetcher);
    reload();
}
...
if (typeof document !== 'undefined') {
    document.addEventListener('click', async event => {
        ...
        if (!globalThis.PublicKeyCredential || !navigator.credentials) {
            showMessage(message, 'Dieser Browser unterstützt keine Passkeys.', true);
            return;
        }

        button.disabled = true;
        showMessage(message, 'Passkey wird vorbereitet …');
        try {
            if (loginButton) {
                const username = document.querySelector(loginButton.dataset.usernameInput)?.value;
                await performPasskeyLogin({
                    username,
                    optionsUrl: loginButton.dataset.optionsUrl,
                    resultUrl: loginButton.dataset.resultUrl,
                    successUrl: loginButton.dataset.successUrl,
                });
            } else {
                await performPasskeyRegistration({
                    optionsUrl: registerButton.dataset.optionsUrl,
                    resultUrl: registerButton.dataset.resultUrl,
                });
            }
        } catch (error) {
            showMessage(message, error instanceof Error ? error.message : 'Passkey-Vorgang abgebrochen.', true);
