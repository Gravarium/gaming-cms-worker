...
final class AccessRoleSecurityTest extends WebTestCase
{
    ...
    public function testRoleDeletionRequiresCsrf(): void
    {
        ...
        $client->request('POST', '/admin/access-roles/'.$role->getId().'/delete');
        self::assertResponseStatusCodeSame(403);
        self::assertNotNull($client->getContainer()->get(AccessRoleRepository::class)->find($role->getId()));

        $token = $client->getContainer()->get(CsrfTokenManagerInterface::class)->getToken('delete-access-role-'.$role->getId())->getValue();
        $client->request('POST', '/admin/access-roles/'.$role->getId().'/delete', ['_token' => $token]);
        self::assertResponseRedirects('/admin/access-roles');
        self::assertNull($client->getContainer()->get(AccessRoleRepository::class)->find($role->getId()));
    }
