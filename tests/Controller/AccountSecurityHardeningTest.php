...
final class AccountSecurityHardeningTest extends WebTestCase
{
    ...
    public function testPasswordChangeRevokesResetTokenOtherSessionsAndTracksMigratedSession(): void
    {
        ...
        $oldTrackedId = $oldTracked->getId();
        self::assertNotNull($oldTrackedId);
        ...
        $em = $this->em($client);
        $em->clear();
        $storedOther = $em->find(UserSession::class, $otherSessionId);
        $storedOld = $em->find(UserSession::class, $oldTrackedId);
        self::assertInstanceOf(UserSession::class, $storedOther);
        self::assertInstanceOf(UserSession::class, $storedOld);
        self::assertTrue($storedOther->isRevoked());
        self::assertTrue($storedOld->isRevoked());
    ...
    public function testOwnSessionRevocationRejectsForeignSessionBeforeCsrfMutation(): void
    {
        $client = static::createClient();
        ...
        $client->request('POST', '/account/security/sessions/'.$foreignId.'/revoke');

        self::assertResponseStatusCodeSame(404);
        $stored = $client->getContainer()->get(UserSessionRepository::class)->find($foreignId);
