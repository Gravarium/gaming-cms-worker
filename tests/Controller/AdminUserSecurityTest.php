...
final class AdminUserSecurityTest extends WebTestCase
{
    public function testUserManagerCannotManageFullAdministratorAccount(): void
    {
        ...
        $client->request('POST', '/admin/users/'.$admin->getId().'/send-verification');
        self::assertResponseStatusCodeSame(403);
    ...
    public function testOwnEmailChangeRequiresCurrentPassword(): void
    {
        ...
        self::assertResponseStatusCodeSame(422);
        self::assertSelectorTextContains('body', 'aktuellen Passwort');
    ...
    public function testEmailChangeRevokesOldAccountTokensAndExistingSessions(): void
    {
        ...
        $this->em($client)->clear();
        $stored = $this->em($client)->find(User::class, $targetId);
        $storedSession = $this->em($client)->find(UserSession::class, $otherSessionId);
        self::assertInstanceOf(User::class, $stored);
    ...
    public function testAdministrativePasswordChangeRevokesResetTokenAndExistingSessions(): void
    {
        ...
        $this->em($client)->clear();
        $storedTarget = $this->em($client)->find(User::class, $targetId);
        $storedSession = $this->em($client)->find(UserSession::class, $sessionId);
        self::assertInstanceOf(User::class, $storedTarget);
        self::assertInstanceOf(UserSession::class, $storedSession);
        self::assertTrue($storedSession->isRevoked());
        self::assertSame($oldVersion + 1, $storedTarget->getSecurityVersion());
        self::assertTrue($this->hasher($client)->isPasswordValid($storedTarget, 'New-Password-84'));
    }
    ...
    public function testAdminSessionRevocationRejectsSessionOwnedByDifferentTarget(): void
    {
        ...
        $this->em($client)->flush();
        $foreignSessionId = $foreignSession->getId();
        self::assertNotNull($foreignSessionId);
        $client->loginUser($actor);

        $client->request('POST', '/admin/users/'.$first->getId().'/sessions/'.$foreignSessionId.'/revoke');

        self::assertResponseStatusCodeSame(404);
        $stored = $client->getContainer()->get(UserSessionRepository::class)->find($foreignSessionId);
