...
final class PublishScheduledContentCommand extends Command
{
    ...
    {
        $now = new \DateTimeImmutable(); $published = 0; $unpublished = 0; $releaseCount = 0; $releaseEntries = 0; $releaseFailures = 0;
        ...
        foreach ($this->releases->findDue($now) as $release) {
            if (!$release->isDue($now)) { continue; }
            try {
                $releaseEntries += $release->publish($now);
                ++$releaseCount;
            } catch (\DomainException $exception) {
                ++$releaseFailures;
                $output->writeln(sprintf('<error>Release %d konnte nicht sicher veröffentlicht werden: %s</error>', $release->getId() ?? 0, $exception->getMessage()));
            }
        }
        $this->entityManager->flush();
        $output->writeln(sprintf('<info>%d veröffentlicht, %d archiviert, %d Releases mit %d Inhalten verarbeitet; %d Releases offen.</info>', $published, $unpublished, $releaseCount, $releaseEntries, $releaseFailures));
        return $releaseFailures === 0 ? Command::SUCCESS : Command::FAILURE;
    }
}
