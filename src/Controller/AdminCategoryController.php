...
final class AdminCategoryController extends AbstractController
{
    ...
    {
        return $this->form(new Category(), $request, 'Kategorie anlegen');
    }

    #[Route('/{id}/edit', name: 'app_admin_category_edit', requirements: ['id' => '\d+'], methods: ['GET', 'POST'])]
    public function edit(Category $category, Request $request): Response
    {
        return $this->form($category, $request, 'Kategorie bearbeiten');
    }
    ...
    private function form(Category $category, Request $request, string $heading): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->entityManager->flush();
            $this->addFlash('success', 'Die Kategorie wurde gespeichert.');
