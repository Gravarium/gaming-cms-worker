...
final class LiveEmbedPolicy
{
    ...
    {
        if(!$stream->isEnabled()||!$consented)return null;
        ...
        $parent=strtolower(preg_replace('/:\d+$/','',$parentHost)?:'localhost');
        if(preg_match('/^[a-z0-9.-]+$/D',$parent)!==1)$parent='localhost';
    ...
    {
        if(!($host==='youtu.be'||$host==='www.youtu.be'||$host==='youtube.com'||str_ends_with($host,'.youtube.com')))return null;
        if($host==='youtu.be'||$host==='www.youtu.be')$id=$path;elseif(preg_match('~^(?:live|embed)/([A-Za-z0-9_-]{6,20})$~',$path,$m)===1)$id=$m[1];else return null;
        if(preg_match('/^[A-Za-z0-9_-]{6,20}$/D',$id)!==1)return null;
        return ['provider'=>'youtube','url'=>'https://www.youtube-nocookie.com/embed/'.$id];
    }
    /** @return array{provider:string,url:string}|null */
    private function vimeo(string $host,string $path):?array
    {
        if(!($host==='vimeo.com'||str_ends_with($host,'.vimeo.com'))||preg_match('~^(\d+)$~',$path,$m)!==1)return null;
        return ['provider'=>'vimeo','url'=>'https://player.vimeo.com/video/'.$m[1]];
    }
    /** @return array{provider:string,url:string}|null */
    private function twitch(string $host,string $path,string $parent):?array
    {
        if(!($host==='twitch.tv'||str_ends_with($host,'.twitch.tv'))||preg_match('/^[A-Za-z0-9_]+$/D',$path)!==1)return null;
