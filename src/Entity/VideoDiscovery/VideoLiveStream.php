...
class VideoLiveStream
{
    ...
    private ?CreatorProfile $creator = null;
    ...
    private string $consentMode = self::CONSENT_CLICK;
    ...
    public function __construct(string $title, string $provider, string $sourceUrl)
    {
        $title = trim($title);
        ...
        if ($title === ''
            ...
            || $sourceUrl === ''
            || mb_strlen($sourceUrl) > 700
        ) {
            throw new \InvalidArgumentException('Invalid live stream.');
        }
    ...
    public function getConsentMode(): string { return $this->consentMode; }
