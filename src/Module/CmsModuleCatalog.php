...
final class CmsModuleCatalog
{
    ...
    public function all(): array
    {
        $definitions = [
            ...
            $this->module('content', 'Inhalte und Navigation', false, ['core'], ['app_admin_content', 'app_admin_category', 'app_admin_menu', 'app_page', 'app_news', 'app_content_search']),
        ...
        $indexed = [];
        foreach ($definitions as $definition) { $indexed[$definition['key']] = $definition; }
        return $indexed;
    }
     ...
     */
    private function module(string $key, string $name, bool $required, array $dependencies, array $routePrefixes): array
    {
        return compact('key', 'name', 'required', 'dependencies', 'routePrefixes') + ['version' => '1.0.0'];
    }
}
